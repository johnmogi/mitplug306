<?php
// Script to find a product by name using the correct wp-config.php file

// Path to the saved wp-config.php path
$config_path_file = __DIR__ . '/wp-config-path.txt';

if (!file_exists($config_path_file)) {
    die("Error: wp-config-path.txt not found. Please run locate-wp-config.php first.\n");
}

$wp_config_path = trim(file_get_contents($config_path_file));
...
$db_pass = DB_PASSWORD;
$db_host = DB_HOST;
$table_prefix = $table_prefix; // This should be defined in wp-config.php

// Product to search for
$search_term = 'מגה סלייד דקלים';

echo "=== Product Search ===\n";
echo "Using wp-config.php from: $wp_config_path\n";
...
try {
    ...
    // Check if the posts table exists with the configured prefix
